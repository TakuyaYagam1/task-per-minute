...
func InitializeApp(ctx context.Context, cfg *config.Config, log logkit.Logger) (*App, func(), error) {
	...
	wsHandshakeLimiter := provideHandshakeRateLimiter(ctx, cfg)
	wireRawWebSocketServer := provideRawWebSocketServer(ctx, cfg, playerPostgres, matchmakingUsecase, flagSubmitUsecase, hubRegistry, hintScheduler, duelPostgres, seaweedStorage, wsHandshakeLimiter)
	...
	server := provideRESTServer(playerUsecase, authUsecase, taskUsecase, uploadUsecase, leaderboardUsecase, readUsecase, healthChecks, loginRateLimiter, joinRateLimiter)
